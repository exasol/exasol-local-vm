...
### DB port is only reachable via `localhost` or `[::1]`, not `127.0.0.1`

When the launcher publishes the DB port (default `8563`), Windows users
must connect to the DB using the hostname `localhost` — not the literal
IPv4 address `127.0.0.1`. A raw `127.0.0.1:8563` connection is refused
with `TCP connect ... failed` even though `ping 127.0.0.1` succeeds and
the container is fully up and serving.

**Why this happens.** Podman-for-windows uses a WSL2 backing VM plus a
Windows-side port-forwarder called `gvproxy`. `gvproxy` binds the
forwarded ports on the IPv6 loopback (`[::1]`) only — nothing listens
on the IPv4 loopback (`127.0.0.1`). This is a design choice in the
current podman-for-windows stack, not a WSL2 limitation. `netstat -ano`
on Windows confirms only the `[::1]:8563` listener; `Test-NetConnection
127.0.0.1 8563` from PowerShell fails; `Test-NetConnection localhost
8563` succeeds.

**Why `localhost` works.** Windows resolves `localhost` to both `::1`
and `127.0.0.1`, and the standard client dial logic (Go's `net.Dial`,
JDBC, .NET, PowerShell, most SQL client GUIs) tries each address in
Happy-Eyeballs order. Because `[::1]:8563` is listening, the connection
succeeds. This is why the launcher's `Publishing db:` message and the
integration tests all use `localhost` and not `127.0.0.1`.

**What breaks anyway.** SQL clients or scripts that hard-code the
literal string `127.0.0.1` in their connection URL will fail on
Windows. For example:

```python
# Fails on Windows, works on mac
conn = f"exa+websocket://SYS:pw@127.0.0.1:8563"
```

```python
# Works everywhere
conn = f"exa+websocket://SYS:pw@localhost:8563"
```

**Workarounds if you must have IPv4 loopback.** None ship with the
launcher today. Three approaches are known to work but each has a
cost:

1. **Edit `%UserProfile%\.wslconfig`** to enable WSL 2 mirrored-mode
   networking (`[wsl2]` → `networkingMode=mirrored`), then
   `wsl --shutdown` and restart the podman machine. Requires Windows
   11 22H2 or later; effect on gvproxy binding is untested by us but
   documented by Microsoft to reshape the loopback story. See
   [Microsoft's WSL networking notes](https://learn.microsoft.com/en-us/windows/wsl/networking#mirrored-mode-networking).
2. **Add a Windows-side v4→v6 port proxy** using
   `netsh interface portproxy add v4tov6 listenaddress=127.0.0.1
   listenport=8563 connectaddress=::1 connectport=8563`. Works, but
   requires **admin** to run `netsh` — defeats the launcher's
   otherwise non-admin install story (Phase 14 uses
   `winget install RedHat.Podman --scope user` specifically to avoid
   UAC). Per-port setup and teardown.
3. **Pass `-p 127.0.0.1:8563:8563`** to `podman run` (explicit IPv4
   bind on the container side). Not currently attempted by the
   launcher; empirically `gvproxy` may still expose the port only on
   `[::1]` regardless of the container-side bind. Worth a small
   experiment but not confirmed to work.

For now the recommendation is: **use `localhost` in every connection
URL**. This costs nothing, matches what the launcher prints, and works
