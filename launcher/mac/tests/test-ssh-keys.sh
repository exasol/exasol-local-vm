...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    if ssh -i "$TEST_KEY" -p "$SSH_PORT" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=5 root@127.0.0.1 "echo 'SSH key import successful!'" 2>/dev/null; then
